...
const groupIssuesByGuideline = (issues: Issue[]) => {
  const groups: Record<string, { label: string, description?: string, issues: Issue[] }> = {};
  ...
  issues.forEach(issue => {
    ...
    if (!groups[guidelineKey]) {
      const desc = guidelineData && 'description' in guidelineData ? (guidelineData as { description: string }).description : undefined;
      groups[guidelineKey] = {
...
const IssueTable: FC<{ issues: Issue[] }> = ({ issues }) => {
  const criterionGroups: Record<string, {
    criterion: string,
    label: string,
    type: string,
    level: string,
  ...
  issues.forEach(issue => {
    ...
    if (!criterionGroups[criterionKey]) {
      const level = criterionData && 'wcagLevel' in criterionData ? (criterionData as { wcagLevel: string }).wcagLevel : 'N/A';
      criterionGroups[criterionKey] = {
        criterion: criterionKey,
        label: criterionData?.label || criterionKey,
        type: issue.severity === 'error' ? 'Fehler' : (issue.severity === 'warning' ? 'Hinweis' : 'Hinweis'),
        level: level,
        count: 0
      };
      if (issue.severity === 'error') criterionGroups[criterionKey].type = 'Fehler';
      else if (issue.severity === 'warning') criterionGroups[criterionKey].type = 'Warnung';
      else criterionGroups[criterionKey].type = 'Hinweis';
    }
    criterionGroups[criterionKey].count += issue.count;
  });

  const tableData = Object.values(criterionGroups).map(g => ({
